...
def main():
	...
	#+fred
	#required_flags.add_argument('-ar','--region', action="store", default=None, help='ewelink api region')

	# Flags with default values go here.
	parser.add_argument('-ar','--region', action="store", default=None, help='ewelink api region')
	...
	parser.add_argument('-d','--device', action="store", default='default', help='Device id of target Device, can be index number, name or deviceid (default: %(default)s)')
	...
	parser.add_argument('-pt','--pub_topic', action="store",default='/ewelink_status/', help='topic to publish ewelink data to. (default: %(default)s)')
	parser.add_argument('-st','--sub_topic', action="store",default='/ewelink_command/', help='topic to publish ewelink commands to. (default: %(default)s)')
	...
	if not arg.region:
			log.critical('You must define a region with command line argument')
			parser.print_help()
			sys.exit(2)
	...
	try:
		...
		client = T0_ewelink_client.EwelinkClient(arg.phone, arg.email, arg.e_password, arg.imei, arg.region, loop, mqttc, arg.pub_topic, arg.config)
