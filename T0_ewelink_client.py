...
class EwelinkClient():
	...
	def __init__(self, phoneNumber=None,email=None,password=None, imei='01234567-89AB-CDEF-0123-456789ABCDEF', region=None, loop=None, mqttc=None, pub_topic='/ewelink_status/', configuration_file=None):
		...
		#region argument
		self.apiHost = '{}-api.coolkit.cc:8080'.format(str(region))
		self.webSocketApi = '{}-pconnect3.coolkit.cc'.format(str(region))
		...
		self._configuration = {}
		self._phoneNumber, self._email, self._password, self._imei, self._region, self._pub_topic = self.get_initialization_parameters(configuration_file, phoneNumber, email, password, imei, region, pub_topic)
		...
		if self._region is None :
			self.logger.error('region cannot be empty')
			sys.exit(1)
	...
	def get_initialization_parameters(self, file, phoneNumber, email, password, imei, region, pub_topic):
		...
		if file:
			if self.load_config(file):
				...
				return my_phoneNumber, my_email, my_password, my_imei, my_pub_topic, my_region

		return phoneNumber, email, password, imei, pub_topic, region
